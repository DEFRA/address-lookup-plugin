...
import { logger } from '~/src/plugins/postcode-lookup/logger.js'
import { getErrorMessage } from '~/src/plugins/postcode-lookup/utils.js'
...
async function getAddressData(url, endpoint) {
  ...
  try {
    const response = await getJsonByType(new URL(url))
...
/**
 * @import { Address, DeliveryPointAddress, DeliveryPointAddressResult } from '~/src/plugins/postcode-lookup/types.js'
 */
